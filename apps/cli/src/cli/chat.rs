use super::core::{local_cli_core, CliCore};
use super::*;
use crate::bootstrap::create_cli_application;
use operit_runtime::core::application::OperitApplication::OperitApplication;

pub(super) async fn run_chat_command(args: &[String]) -> Result<(), String> {
    if args.is_empty() {
        print_chat_usage();
        return Ok(());
    }
    run_chat_command_with_core(&mut local_cli_core()?, args).await
}

pub(super) async fn run_chat_command_with_core(
    core: &mut CliCore,
    args: &[String],
) -> Result<(), String> {
    if args.is_empty() {
    ...
    match args[0].as_str() {
        "new" => create_chat_with_core(core, &args[1..]).await,
        "list" => list_chats_with_core(core).await,
