...
exports.onPlanModeCommand = onPlanModeCommand;
...
async function onPlanModeCommand(event) {
    ...
    if (action === "status") {
        ...
        const enabled = await plan_mode_runtime_ipc_js_1.PlanModeShared.isEnabled(activeView.chatId);
        return {
    ...
    if (action === "off") {
        ...
        await plan_mode_runtime_ipc_js_1.PlanModeShared.disable(activeView.chatId);
        return {
    ...
    const workspace = await plan_mode_runtime_ipc_js_1.PlanModeShared.resolveWorkspace(activeView.chatId, activeView.runtime);
    if (!workspace) {
        return {
...
async function enablePlanModeFromCommand(text) {
    const activeView = await plan_mode_runtime_ipc_js_1.PlanModeShared.getSingleActiveChatView();
    if (!activeView) {
        return {
            stderr: text.commandChatViewMissing,
...
async function handleSubmitPlanaskAnswersIpc(request) {
    const text = (0, plan_mode_i18n_js_1.resolvePlanModeI18n)();
    try {
        const trackedView = await plan_mode_runtime_ipc_js_1.PlanModeShared.getTrackedChatViewByChatId(request.chatId);
        if (!trackedView) {
        ...
        const workspace = await plan_mode_runtime_ipc_js_1.PlanModeShared.resolveWorkspace(trackedView.chatId, trackedView.runtime);
        if (!workspace) {
...
async function handleStartImplementationIpc(request) {
    const text = (0, plan_mode_i18n_js_1.resolvePlanModeI18n)();
    ...
    try {
        ...
        if (!(await plan_mode_runtime_ipc_js_1.PlanModeShared.claimPlanStart(normalizedPlanContent, trackedView.chatId))) {
        ...
        try {
            ...
            await plan_mode_runtime_ipc_js_1.PlanModeShared.forgetPlanStart(normalizedPlanContent, trackedView.chatId);
            throw error;
        }
    }
    catch (error) {
...
async function handleIsPlanStartedIpc(request) {
    try {
        return await plan_mode_runtime_ipc_js_1.PlanModeShared.isPlanStarted(request.planContent, request.chatId);
    }
    catch (error) {
