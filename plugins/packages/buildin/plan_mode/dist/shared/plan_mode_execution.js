 ...
 */
async function isPlanImplementationStarted(planContent, chatId) {
    const normalizedPlanContent = planContent.trim();
    if (!normalizedPlanContent) {
        return false;
    }
    try {
        return await ToolPkg.ipc.call(exports.PLAN_MODE_IS_PLAN_STARTED_IPC_CHANNEL, { planContent: normalizedPlanContent, chatId });
    }
    catch (error) {
...
/** Requests plan persistence, mode exit, and implementation kickoff. */
async function startPlanImplementation(planContent, chatId) {
    ...
    try {
        return await ToolPkg.ipc.call(exports.PLAN_MODE_START_IMPLEMENTATION_IPC_CHANNEL, { planContent: normalizedPlanContent, chatId });
    }
    catch (error) {
        const errorText = error instanceof Error
            ? error.message || "error"
