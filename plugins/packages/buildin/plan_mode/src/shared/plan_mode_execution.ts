 ...
 */
export async function isPlanImplementationStarted(planContent: string, chatId: string): Promise<boolean> {
  const normalizedPlanContent = planContent.trim();
  if (!normalizedPlanContent) {
    return false;
  }
  try {
    return await ToolPkg.ipc.call<PlanImplementationRequest, boolean>(
      PLAN_MODE_IS_PLAN_STARTED_IPC_CHANNEL,
      { planContent: normalizedPlanContent, chatId }
    );
  } catch (error) {
...
export async function startPlanImplementation(
  ...
  try {
    return await ToolPkg.ipc.call<PlanImplementationRequest, StartPlanImplementationResult>(
