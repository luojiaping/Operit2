...
function screen(ctx) {
    ...
    async function removeNode() {
        ...
        if (dependents.length)
            throw new Error("先修改这些节点的参数引用：" + dependents.map(node => node.name).join("、"));
    ...
    function listView() {
        if (!state.ready)
            return UI.Box({ fillMaxSize: true, contentAlignment: "center" }, state.error ? button("重新加载", initialize) : UI.CircularProgressIndicator());
        ...
        if (state.workflow === null) {
            if (state.selectionMode) {
                if (state.marked.length)
                    actions.push({ label: "删除所选 (" + state.marked.length + ")", icon: "Delete", danger: true, action: () => update({ modal: "delete" }) });
                actions.push({ label: "退出多选", icon: "CheckCircle", action: () => update({ selectionMode: false, marked: [] }) });
            }
            else
                actions.push({ label: "创建空白工作流", icon: "Add", action: () => update({ modal: "create", name: "", description: "" }) }, { label: "从模板创建", icon: "PlayCircle", action: () => update({ modal: "templates" }) }, { label: "多选", icon: "CheckCircle", action: () => update({ selectionMode: true, marked: [] }) }, { label: "导入工作流", icon: "FileUpload", action: () => update({ modal: "import", text: "" }) });
        }
        else {
            if (state.workflow.enabled || state.busy)
                actions.push({ label: state.busy ? "取消执行" : "触发工作流", icon: state.busy ? "Close" : "PlayArrow",
                    action: state.busy ? async () => { await request({ action: "cancel", id: current().id }); } : run });
            actions.push({ label: "查看日志", icon: "Call", action: async () => { await request({ action: "list" }); update({ modal: "logs", logId: null, logNode: null }); } }, { label: "添加节点", icon: "Add", enabled: !state.busy, action: () => showNode(null) }, { label: "编辑工作流", icon: "Edit", enabled: !state.busy, action: () => {
                    const workflow = current();
    ...
    function logs(workflow) {
        ...
        if (selected === undefined)
            return [UI.Text({ text: "暂无执行日志" })];
        return [
            ...(runs.length > 1 ? [(0, forms_1.choose)(ctx, "run-history", "执行记录", selected.id, runs.map(run => ({ value: run.id, label: date(run.startedAt) + " · " + status(run.status).text })), logId => update({ logId }))] : []),
    ...
    function modal() {
        ...
        switch (state.modal) {
            ...
            case "meta": return [dialog("meta", "编辑工作流", [
                    ...
                    UI.Row({ fillMaxWidth: true, verticalAlignment: "center" }, [UI.Text({ text: "启用工作流", weight: 1 }), UI.Switch({ checked: state.enabled, onCheckedChange: (enabled) => update({ enabled }) })]),
            ...
            case "node": {
                ...
                if (draft === null || workflow === null)
                    throw new Error("节点编辑上下文不存在");
                const dialogs = [dialog("node:" + draft.id, state.adding ? "添加节点" : "编辑节点", [
                        ...(state.adding ? [(0, forms_1.choose)(ctx, "node-type", "节点类型", draft.type, Object.keys(model_1.STYLES).map(value => ({ value, label: model_1.STYLES[value].label + "节点" })), value => {
                ...
                if (schedule !== null && schedule.type === "trigger")
                    dialogs.push(dialog("schedule", "定时配置", (0, forms_1.scheduleForm)(ctx, schedule, value => update({ schedule: value })), [
            ...
            case "condition": {
                ...
                return [dialog("condition", "编辑连接条件", [
                        ...
                        ...(state.conditionMode === "custom" ? [(0, forms_1.field)(ctx, "edge-condition", "正则表达式 / on_success / on_error", state.text, text => update({ text }))] : []),
            ...
            case "export": return [dialog("export", "导出工作流", [
                    ...
                    (0, forms_1.field)(ctx, "export-path", "保存路径", state.path, path => update({ path })),
                ], [close, button("导出", async () => {
                        await Tools.Files.create(live.current.path, live.current.text);
                        await ctx.showToast("已导出工作流");
                        dismiss();
