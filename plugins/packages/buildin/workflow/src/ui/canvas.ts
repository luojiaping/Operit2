import type { ComposeCanvasCommand, ComposeColor, ComposeDslContext, ComposeNode } from "../../../../../types/compose-dsl";
import { STYLES, values, type Workflow, type WorkflowNode, type Run } from "../model";
import { edgeMatches } from "../engine";

export interface Viewport { x: number; y: number; zoom: number; width: number; height: number }
export interface CanvasActions {
  viewport: Viewport; dragging: string | null; run: Run | null;
  menu(id: string): void; fit(): void;
  begin(id: string | null): void; drag(dx: number, dy: number): void; end(): void | Promise<void>; cancel(): void;
  transform(x: number, y: number, zoom: number): void; resize(width: number, height: number): void;
}
...
export function fitViewport(workflow: Workflow, viewport: Viewport): Viewport {
  if (!workflow.nodes.length || viewport.width <= 0 || viewport.height <= 0) return viewport;
...
export function hitNode(workflow: Workflow, x: number, y: number): string | null {
  for (let i = workflow.nodes.length - 1; i >= 0; i--) {
    const node = workflow.nodes[i];
    if (x >= node.position.x && x <= node.position.x + 120 && y >= node.position.y && y <= node.position.y + 80) return node.id;
  }
  return null;
}

/** Intersects the center-to-center direction with the two node boundaries. */
export function edgePoints(a: WorkflowNode, b: WorkflowNode): { x: number; y: number }[] {
...
function curve(ax: number, ay: number, bx: number, by: number): { x: number; y: number }[] {
...
export function graphCanvas(ctx: ComposeDslContext, workflow: Workflow, actions: CanvasActions): ComposeNode {
  ...
  const sx = (x: number): number => v.x + x * v.zoom;
  /** Converts graph-space vertical coordinates into canvas-space positions. */
  const sy = (y: number): number => v.y + y * v.zoom;
  /** Centers bounded node labels using host text measurement. */
  function centered(text: string, x: number, y: number, fontSize: number, maxWidth: number, color: ComposeColor, maxLines = 1): void {
  ...
  function arrow(a: { x: number; y: number }, b: { x: number; y: number }, color: ComposeColor): void {
  ...
  function drawEdge(a: WorkflowNode, b: WorkflowNode, color: ComposeColor, dashed: boolean, label: string | null, reference = false): void {
    ...
    for (let index = 1; index < points.length; index++) {
      ...
      if (!dashed || length % (12 * v.zoom) < 7 * v.zoom) commands.push({ type: "Line", x1: previous.x, y1: previous.y, x2: point.x, y2: point.y, color, strokeWidth: 1.5 * v.zoom });
    }
    arrow(points[63], points[64], color);
    if (!reference) arrow(points[31], points[32], color);
    if (label !== null) {
  ...
  for (let x = ((v.x % grid) + grid) % grid; x < v.width; x += grid) for (let y = ((v.y % grid) + grid) % grid; y < v.height; y += grid) {
    commands.push({ type: "Circle", cx: x, cy: y, radius: 1.5 * v.zoom, color: "#888888", filled: true });
  }
  for (const target of workflow.nodes) {
    const references = new Set(values(target).flatMap(value => "nodeId" in value ? [value.nodeId] : []));
    for (const sourceId of references) drawEdge(workflow.nodes.find(node => node.id === sourceId)!, target, "#BFFF9800", true, null, true);
  }
  for (const edge of workflow.connections) {
    ...
    const active = sourceResult !== undefined && targetResult !== undefined && targetResult.status !== "skipped" && edgeMatches(edge, a, sourceResult);
  ...
  for (const node of workflow.nodes) {
    ...
    let border: ComposeColor = actions.dragging === node.id ? style.color : style.border;
    if (result) border = { pending: border, running: "#2196F3", success: "#4CAF50", failed: "#F44336", skipped: "#9E9E9E" }[result.status];
    commands.push(
      { type: "RoundRect", x, y: y + 2 * v.zoom, width, height, radius: 8 * v.zoom, color: "#20000000", filled: true },
      { type: "RoundRect", x, y, width, height, radius: 8 * v.zoom, color: actions.dragging === node.id ? style.tint : "#FFFFFF", filled: true },
      { type: "RoundRect", x, y, width, height, radius: 8 * v.zoom, color: border, filled: false, strokeWidth: (result ? 3 : 2) * v.zoom },
      { type: "RoundRect", x: x + 7 * v.zoom, y: y + 6 * v.zoom, width: width - 14 * v.zoom, height: 17 * v.zoom, radius: 4, color: style.tint, filled: true },
      { type: "DrawIcon", icon: node.type === "trigger" ? "PlayArrow" : "Settings", x: x + 40 * v.zoom, y: y + 9 * v.zoom, size: 12 * v.zoom, color: style.color },
    );
  ...
  return ctx.UI.Canvas({ key: "workflow-canvas", fillMaxSize: true, background: "#F8F9FA",
    ...
    modifier: ctx.Modifier.fillMaxSize().clipToBounds()
      .tapGestures({ onDoubleTap: actions.fit, onLongPress: point => {
        ...
        if (node !== null) actions.menu(node);
      } })
      ...
      .transformGestures({ onGesture: event => {
        ...
        actions.transform(event.centroidX - (event.centroidX - event.panX - v.x) * ratio,
          event.centroidY - (event.centroidY - event.panY - v.y) * ratio, zoom);
      } }),
  });
}
