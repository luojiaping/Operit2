import { id, type Workflow, type WorkflowNode, type Value, type NodeResult, type Run, type Connection, type Condition, type Extract } from "./model";
import { validateGraph } from "./validation";

export interface ExecutionHost {
  call(name: string, params: Record<string, string>): Promise<string>;
  script(code: string, inputs: Record<string, string>, trigger: Record<string, string>): Promise<string>;
  changed(run: Run): Promise<void>;
  cancelled(): boolean;
}
...
export function resolve(value: Value, results: Record<string, NodeResult>): string {
  if ("value" in value) return value.value;
  const source = results[value.nodeId];
  if (source === undefined || source.status !== "success") throw new Error(`引用节点未成功完成：${value.nodeId}`);
...
export function compare(node: Condition, results: Record<string, NodeResult>): boolean {
  const a = resolve(node.left, results), b = resolve(node.right, results);
  if (node.operator === "CONTAINS") return a.includes(b);
  if (node.operator === "NOT_CONTAINS") return !a.includes(b);
  if (node.operator === "IN" || node.operator === "NOT_IN") {
    const list: unknown = JSON.parse(b);
    if (!Array.isArray(list) || !list.every(value => typeof value === "string") && !list.every(value => typeof value === "number")) throw new Error("IN 的右值必须是同类型字符串或数字 JSON 数组");
  ...
  if (an !== bn) throw new Error("条件比较的左右值类型不一致");
  const left = an ? Number(a) : a, right = bn ? Number(b) : b;
  switch (node.operator) {
    case "EQ": return left === right; case "NE": return left !== right;
...
export function jsonPath(source: string, path: string): string {
  let result: unknown = JSON.parse(source);
  let cursor = path.startsWith("$") ? 1 : 0;
  while (cursor < path.length) {
    const match = /^(?:\.?([A-Za-z_$][\w$]*)|\[(\d+)\])/.exec(path.slice(cursor));
    if (!match) throw new Error(`JSON 路径无效：${path}`);
    const key = match[1] === undefined ? match[2] : match[1];
    if (result === null || typeof result !== "object" || !Object.prototype.hasOwnProperty.call(result, key)) throw new Error(`JSON 路径不存在：${path}`);
    result = (result as Record<string, unknown>)[key]; cursor += match[0].length;
  }
  if (path === "") throw new Error("JSON 路径不能为空，整个对象请使用 $");
  return typeof result === "string" ? result : JSON.stringify(result);
}

/** Executes all six extraction modes, reporting malformed or missing data. */
export function extract(node: Extract, results: Record<string, NodeResult>): string {
  switch (node.mode) {
    case "RANDOM_INT": {
      if (node.useFixed) { if (!/^-?\d+$/.test(node.fixedValue) || !Number.isSafeInteger(Number(node.fixedValue))) throw new Error("固定值必须是整数"); return node.fixedValue; }
    ...
    case "SUB": {
      ...
      if (node.startIndex > source.length) throw new Error("截取起点超出字符串长度");
    ...
    case "REGEX": {
      ...
      if (match === null || match[node.group] === undefined) throw new Error("正则表达式未匹配到指定分组");
...
export function edgeMatches(edge: Connection, source: WorkflowNode, result: NodeResult): boolean {
  if (result.status === "skipped") return false;
  let condition = edge.condition === null ? "" : edge.condition.trim();
  if (condition === "" && (source.type === "condition" || source.type === "logic")) condition = "true";
  switch (condition.toLowerCase()) {
    case "on_error": case "error": case "failed": return result.status === "failed";
    case "": case "on_success": case "success": case "ok": return result.status === "success";
...
export async function execute(workflow: Workflow, triggerId: string | null, extras: Record<string, string>, host: ExecutionHost): Promise<Run> {
  const run: Run = { id: id("run"), workflowId: workflow.id, workflowName: workflow.name, triggerId,
    status: "RUNNING", startedAt: Date.now(), finishedAt: null, nodes: {}, logs: [] };
  /** Stores one observable node transition with a timestamped log. */
  async function transition(node: WorkflowNode, status: NodeResult["status"], output: string): Promise<void> {
  ...
  try {
    ...
    if (!triggers.length) throw new Error(triggerId === null ? "工作流没有手动触发节点" : "指定触发节点不存在");
    const active = new Set(triggers.map(node => node.id));
    const queue = [...active];
    for (let i = 0; i < queue.length; i++) for (const edge of workflow.connections.filter(edge => edge.sourceNodeId === queue[i])) {
      if (!active.has(edge.targetNodeId)) { active.add(edge.targetNodeId); queue.push(edge.targetNodeId); }
    }
    for (const node of workflow.nodes) run.nodes[node.id] = { status: active.has(node.id) ? "pending" : "skipped", output: "", startedAt: null, finishedAt: null };
    await host.changed(run);
    const pending = workflow.nodes.filter(node => active.has(node.id));
    while (pending.length) {
      if (host.cancelled()) { run.status = "CANCELLED"; break; }
      ...
      if (index < 0) throw new Error("依赖图无法继续执行");
      ...
      try {
        if (incoming.length > 0 && !incoming.some(edge => edgeMatches(edge, nodes.get(edge.sourceNodeId)!, run.nodes[edge.sourceNodeId]))) {
          await transition(node, "skipped", "连线条件未满足"); continue;
        }
        await transition(node, "running", "");
        let output: string;
        switch (node.type) {
          case "trigger": output = JSON.stringify(extras); break;
          case "condition": output = String(compare(node, run.nodes)); break;
          case "logic": {
            ...
            output = String(node.operator === "AND" ? inputs.length > 0 && inputs.every(Boolean) : inputs.some(Boolean)); break;
          }
          case "extract": output = extract(node, run.nodes); break;
          case "execute": {
            const params = Object.fromEntries(Object.entries(node.actionConfig).map(([key, value]) => [key, resolve(value, run.nodes)]));
            output = node.jsCode !== null && node.jsCode.trim() !== "" ? await host.script(node.jsCode, params, extras) : await host.call(node.actionType, params); break;
          }
        }
    ...
    if (run.status !== "CANCELLED") run.status = Object.values(run.nodes).some(node => node.status === "failed") ? "FAILED" : "SUCCESS";
  } catch (error) {
    console.error(`[workflow:${workflow.id}]`, error);
    run.status = "FAILED";
    run.logs.push({ time: Date.now(), nodeId: "", level: "error", message: errorText(error) });
  }
  for (const node of Object.values(run.nodes)) if (node.status === "pending") { node.status = "skipped"; node.output = "执行已取消"; node.finishedAt = Date.now(); }
